...
| Metric            | Value | Notes                                                    |
| ----------------- | ----- | -------------------------------------------------------- |
| Code Coverage | 98% (`copy_folder.py`) / 70% total | Docker run 2026-09-24 (PMO audit, `python:3.12-slim`, `pip install -r requirements-dev.txt -e .`): `copy_folder.py` 98% (419 stmts, 7 miss), `retry.py` 96% (27 stmts, 1 miss), `gcp_setup.py` 0% (181 stmts, untested; see TASKS.md). 127 tests pass in ~8s. Command: `pytest tests/ --cov=gcp --cov-report=term-missing`. Use an editable install (`-e .`) so pytest measures the checked-out `gcp/`. With a plain `pip install .`, tests can import the site-packages copy instead, and `--cov=gcp` may then miss it (the first 2026-09-24 container run reported 0% this way). |
